...
# IOM/POM refactor); native DCPPowerModel/ACPPowerModel and the PowerModels nonlinear
...
@testset "Network DC-PF with VirtualPTDF Model" begin
    ...
    for (sys_name, objval) in (("c_sys5", 240000.0), ("c_sys14", 142000.0))
        sys = PSB.build_system(PSITestSystems, sys_name)
        template = get_thermal_dispatch_template_network(
            NetworkModel(PTDFPowerModel; PTDF_matrix = VirtualPTDF(sys)),
        )
        ps_model = DecisionModel(
            template,
            sys;
...
@testset "Network Unsupported Power Model Formulations" begin
    for network in POM.UNSUPPORTED_POWERMODELS
...
@testset "PTDFPowerModel radial-branch reduction matches unreduced flows" begin
