...
function test_ic_serialization_outputs(model::POM.OperationModel; ic_file_exists, message)
    ic_file = POM.get_initial_conditions_file(model)
    log_file = POM.get_log_file(model)

    @test isfile(ic_file) == ic_file_exists
    if ic_file_exists
        @test Serialization.deserialize(ic_file) isa POM.InitialConditionsData
    end
