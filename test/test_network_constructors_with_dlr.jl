...
"""
Verify that solved branch flows respect Dynamic Line Rating (DLR) constraints.
For each DLR-enabled branch, checks that |flow| ≤ static_rating × dlr_factor at each time step.
Adapted from PSI's check_dlr_branch_flows! to work with POM's in-memory model access.
"""
...
@testset "Network DC-PF with VirtualPTDF Model and implementing Dynamic Branch Ratings" begin
    ...
    for (ix, sys) in enumerate(systems)
        ...
        check_dlr_branch_flows!(ps_model, sys, branches_dlr[sys], dlr_factors, nothing)
    end
end
