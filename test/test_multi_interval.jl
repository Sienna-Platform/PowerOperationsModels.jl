...
@testset "DecisionModel with explicit interval" begin
    ...
    @test solve!(model_24h) == IOM.RunStatus.SUCCESSFULLY_FINALIZED
    ...
    @test build!(model_12h; output_dir = mktempdir(; cleanup = true)) ==
          IOM.ModelBuildStatus.BUILT
    @test solve!(model_12h) == IOM.RunStatus.SUCCESSFULLY_FINALIZED

    # Verify models have the correct interval in store params
    @test IOM.get_interval(model_24h) == Dates.Millisecond(Hour(24))
    @test IOM.get_interval(model_12h) == Dates.Millisecond(Hour(12))
end
...
@testset "Auto-transform SingleTimeSeries with interval" begin
    ...
    @test build!(model2; output_dir = mktempdir(; cleanup = true)) ==
          IOM.ModelBuildStatus.BUILT
    @test solve!(model2) == IOM.RunStatus.SUCCESSFULLY_FINALIZED
end
