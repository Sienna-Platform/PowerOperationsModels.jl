...
function variable_branch_current(
    pm::AbstractIVRModel;
...
function variable_dcline_current(
    pm::AbstractIVRModel;
    nw::Int = nw_id_default,
    bounded::Bool = true,
    report::Bool = true,
...
function constraint_voltage_drop(
    ...
    JuMP.@constraint(
        pm.model,
        vr_to == (vr_fr * tr + vi_fr * ti) / tm^2 - r * csr_fr + x * csi_fr
    )
    JuMP.@constraint(
        pm.model,
        vi_to == (vi_fr * tr - vr_fr * ti) / tm^2 - r * csi_fr - x * csr_fr
    )
end
...
`sum(cr + im*ci) = 0`
"""
function constraint_current_balance(
    pm::AbstractIVRModel,
    n::Int,
    i,
    bus_arcs,
    ...
    JuMP.@constraint(pm.model,
        ...
        -
        (sum(pd for pd in values(bus_pd)) * vr + sum(qd for qd in values(bus_qd)) * vi) /
        (vr^2 + vi^2)
        -
    ...
    JuMP.@constraint(pm.model,
        ...
        -
        (sum(pd for pd in values(bus_pd)) * vi - sum(qd for qd in values(bus_qd)) * vr) /
        (vr^2 + vi^2)
        -
