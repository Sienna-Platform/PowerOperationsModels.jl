...
function variable_shunt_admittance_factor(
    ...
    wz_shunt =
        var(pm, nw)[:wz_shunt] = JuMP.@variable(pm.model,
            [i in ids(pm, nw, :shunt)], base_name = "$(nw)_wz_shunt",
            lower_bound = 0,
            upper_bound =
                ref(pm, nw, :bus)[ref(pm, nw, :shunt, i)["shunt_bus"]]["vmax"]^2,
...
function expression_branch_power_ohms_yt_to(
    ...
    var(pm, n, :p)[t_idx] =
        (g + g_to) * w_to + (-g * tr - b * ti) / tm^2 * wr + (-b * tr + g * ti) / tm^2 * -wi
    var(pm, n, :q)[t_idx] =
...
function constraint_network_power_balance(
    ...
    JuMP.@constraint(
        ...
        sum(
            g_fr * w[i] / tm^2 + g_to * w[j] for
            (i, j, r, x, tm, g_fr, g_to) in values(comp_branch_g)
        )
    )
    JuMP.@constraint(
        pm.model,
        ...
        sum(bs * w[i] for (i, bs) in values(comp_bs)) - sum(
            b_fr * w[i] / tm^2 + b_to * w[j] for
            (i, j, r, x, tm, b_fr, b_to) in values(comp_branch_b)
        )
    )
end
