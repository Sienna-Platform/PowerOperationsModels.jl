...
function constraint_ohms_y_oltc_pst_from(
    ...
    JuMP.@constraint(
        ...
        p_fr ==
        (g + g_fr) / tm^2 * vm_fr^2 +
        (-g) / tm * (vm_fr * vm_to * cos(va_fr - va_to - ta)) +
    ...
    JuMP.@constraint(
        ...
        q_fr ==
        -(b + b_fr) / tm^2 * vm_fr^2 -
        (-b) / tm * (vm_fr * vm_to * cos(va_fr - va_to - ta)) +
...
```
"""
function constraint_power_losses_lb(
    pm::AbstractACPModel,
    n::Int,
    f_bus,
    t_bus,
