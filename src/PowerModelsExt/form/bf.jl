...
"""
function constraint_power_losses(
    pm::AbstractBFModel,
    n::Int,
    i,
    f_bus,
...
function constraint_voltage_magnitude_difference(
    ...
    JuMP.@constraint(
        ...
        2 * (r * p_fr + x * q_fr) -
        (r^2 + x^2) *
        (ccm + ym_sh_sqr * (w_fr / tm^2) - 2 * (g_sh_fr * p_fr - b_sh_fr * q_fr))
    )
end
...
function constraint_model_current(pm::AbstractBFConicModel, n::Int)
    ...
    for (i, branch) in ref(pm, n, :branch)
        ...
        JuMP.@constraint(
            pm.model,
            [w[f_bus] / tm^2, ccm[i] / 2, p[f_idx], q[f_idx]] in
...
function constraint_voltage_angle_difference(
    ...
    JuMP.@constraint(pm.model,
        tan(angmin) *
        ((tr + tzr * g_fr + tzi * b_fr) * (w_fr / tm^2) - tzr * p_fr + tzi * q_fr)
        <=
