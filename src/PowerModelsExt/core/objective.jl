    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i, gen) in ref(pm, n, :gen)
            ...
            if gen["model"] == 1
                ...
                pg_cost[i] = _pwl_cost_expression(
                    pm,
                    pg_terms,
                    points;
                    nw = n,
            ...
            elseif gen["model"] == 2
                ...
                pg_cost[i] = _polynomial_cost_expression(
                    pm,
                    pg_terms,
                    cost_rev;
                    nw = n,
    ...
    for (n, nw_ref) in nws(pm)
        ...
        for (i, dcline) in ref(pm, n, :dcline)
            ...
            if dcline["model"] == 1
                ...
                p_dc_cost[i] = _pwl_cost_expression(
                    pm,
                    p_dc_terms,
                    points;
                    nw = n,
            ...
            elseif dcline["model"] == 2
                ...
                p_dc_cost[i] = _polynomial_cost_expression(
                    pm,
                    p_dc_terms,
                    cost_rev;
                    nw = n,
                ...
                error(
                    "only cost models of types 1 and 2 are supported at this time, given cost model type of $(model) on dcline $(i)",
                )
            end
        end
...
function _pwl_cost_expression(
    pm::AbstractPowerModel,
    x_list::Array{JuMP.VariableRef},
    points;
    nw = 0,
    id = 1,
    var_name = "x",
...
function _polynomial_cost_expression(
    pm::AbstractPowerModel,
    x_list::Array{JuMP.VariableRef},
    cost_terms;
    nw = 0,
    ...
    elseif length(cost_terms) == 3
        ...
        return JuMP.@expression(
            pm.model,
            cost_terms[1] + cost_terms[2] * x + cost_terms[3] * x^2 +
            sum(v * x^(d + 2) for (d, v) in enumerate(cost_nl))
        )
    end
end
...
function _polynomial_cost_expression(
    ...
    elseif length(cost_terms) == 3
        ...
        error(
            "the network cost data features a polynomial cost function that is not compatible with conic mathematical programs.",
        )
    end
end
        ...
        sum(
            (
                time_elapsed[n] * (
                    ...
                    sum(
                        load_weight[n][i] * abs(load["pd"]) * z_demand[n][i] for
                        (i, load) in ref(pm, n, :load)
                    )
                )
            )
            for n in nws)
