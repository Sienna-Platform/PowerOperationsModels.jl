# Event (contingency) constraints: add outage constraints during ModelConstructStage.
# Ported from PowerSimulations.jl `src/contingency_model/contingency_constraints.jl`,
# adapted to POM's type-based accessors and the psy6 explicit-units convention. The active
...
function _add_reactive_power_contingency_constraint_impl!(
    ...
    param_array = get_parameter_array(container, AvailableStatusParameter, V)
...
function _get_reactive_power_upper_bound(device::PSY.ElectricLoad)
    return PSY.get_max_reactive_power(device, PSY.SU)^2
end
