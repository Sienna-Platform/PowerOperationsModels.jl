...
# Arguments
  - `condition_function::Function`: user defined function to determine if event is triggered.
"""
...
"""
    set_event_model!(model::DeviceModel, event_model::EventModel)

Attach `event_model` to `model`, deriving the `EventKey` from the event's contingency type
and the device model's component type.
"""
function set_event_model!(
    model::DeviceModel{D, B},
    event_model::EventModel,
) where {D <: PSY.Component, B <: AbstractDeviceFormulation}
    key = EventKey(get_event_type(event_model), D)
    IOM.set_event_model!(model, key, event_model)
    return
end
